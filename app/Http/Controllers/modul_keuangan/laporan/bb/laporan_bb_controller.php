...
namespace App\Http\Controllers\modul_keuangan\laporan\bb;
...
class laporan_bb_controller extends Controller
{
    public function index(Request $request){
        ...
        if($request->type == 'bbk'){
        	...
        	if(count($data) > 0){
        		if($cek){
	        		...
	        		$nomor = str_pad($urut, 3, "0", STR_PAD_LEFT).'/RJ/BBK/'.date('d', strtotime($tgl)).'/'.date('m', strtotime($tgl)).'/'.date('y', strtotime($tgl));
        	...
        	if(count($data) > 0){
        		if($cek){
	        		...
	        		$urut = ($urt) ? (int) explode('/', $urt->pr_nomor)[0] + 1 : 1;
	        		$nomor = str_pad($urut, 3, "0", STR_PAD_LEFT).'/RJ/BBM/'.date('d', strtotime($tgl)).'/'.date('m', strtotime($tgl)).'/'.date('y', strtotime($tgl));
    	...
    	return view('modul_keuangan.laporan.bb.index',compact('cabang', 'nomor', 'data', 'tgl'));
    }
}
